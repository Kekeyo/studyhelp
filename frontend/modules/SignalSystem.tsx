...
5. 【禁止裸输出任何 LaTeX 数学代码】！凡出现 \\frac, \\dfrac, \\int, \\sum, \\phi, \\alpha, \\sqrt, \\boxed, \\le, \\ge, 上标 ^、下标 _ 等数学语法，必须完整位于 $...$ 或 $$...$$ 内。
6. 禁止使用 \\begin{aligned}、\\begin{cases}、\\begin{matrix}、\\end{...}、&、\\\\ 等多行对齐写法。长推导拆成多条独立的 $$...$$ 公式；每条公式只写一行。
7. $$ 的开始和结束必须各自独占一行，不能夹在正文句子中，也不能在公式内再次插入 $ 或 $$。
8. 不要对 Markdown 符号进行多余转义，禁止输出 \\_、\\*、\\# 等形式。
9. 标题、正文、公式之间保持正常换行。
...
4. 【禁止裸输出任何 LaTeX 数学代码】！凡出现 \\frac, \\dfrac, \\int, \\sum, \\phi, \\alpha, \\sqrt, \\boxed, \\le, \\ge, 上标 ^、下标 _ 等数学内容，都必须位于 $...$ 或 $$...$$ 内。
5. 禁止使用 \\begin{aligned}、\\begin{cases}、\\begin{matrix}、\\end{...}、&、\\\\ 等多行对齐写法。长推导必须拆成多条独立的 $$...$$ 公式，每条公式只写一行。
6. $$ 的开始和结束必须各自独占一行；不能把 $$ 接在正文句尾，不能把 $ 或 $$ 插入一个 LaTeX 公式内部。
...
10. 输出前自行检查：最终内容中不得出现未被数学定界符包裹的 \\int、\\frac、\\phi 等 LaTeX 源码，也不得出现 \\begin、\\end、&、\\\\ 等多行对齐标记。
...
2. 行内公式统一使用 $...$，独立公式统一使用 $$...$$。
3. 严禁裸输出 \\int、\\frac 等命令；禁止使用 \\begin、\\end、&、\\\\ 等多行对齐写法。长推导拆成多条独立的 $$...$$ 公式。
4. 每个 $$ 都必须在独立行开始和结束；标题必须单独成行，与正文、公式之间保持换行。`;
